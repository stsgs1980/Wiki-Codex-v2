/**
 * submitDocument action — POST /api/documents with retry logic.
 * Handles duplicate detection (409), client errors (4xx), server errors (5xx, retryable),
 * and network errors (server down/restarting in sandbox).
 */
import type { UploadState, DuplicateInfo } from './use-upload-state'
...
function normalizeFileType(fileName: string): string {
  ...
  const validTypes = new Set(['md', 'adoc', 'txt', 'html', 'pdf', 'json', 'js', 'ts', 'py', 'yaml', 'yml', 'toml', 'xml', 'css', 'sql', 'sh', 'jsx', 'tsx', 'csv'])
