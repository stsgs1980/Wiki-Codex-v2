  ...
  return (
    <Card className="overflow-hidden transition-shadow hover:shadow-md hover:shadow-terminal-accent/5">
      <CardHeader className="cursor-pointer select-none pb-4 border-b border-border">
        <div className="flex items-start gap-4">
          <button
            ...
            aria-controls={`db-instruction-content-${instruction.id}`}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 flex-wrap mb-1">
                ...
                <span className="text-3xs font-mono px-2 py-0.5 rounded-full border bg-terminal-accent/10 text-terminal-accent border-terminal-accent/20">
                  {steps.length} {steps.length === 1 ? 'step' : 'steps'}
                </span>
                {instruction.sourceDoc && (
                  <span className="text-3xs font-mono px-2 py-0.5 rounded-full border bg-muted text-muted-foreground border-border gap-1 inline-flex items-center">
      ...
      {expanded && steps.length > 0 && (
        <CardContent className="pt-6 pb-6 px-6" id={`db-instruction-content-${instruction.id}`}>
          {steps.map((step, idx) => (
