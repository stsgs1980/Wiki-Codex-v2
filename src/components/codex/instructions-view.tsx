...
const BUILTIN_TEMPLATES: TemplateGroup[] = [
  {
    ...
    description: 'Как сохранить данные, восстановить после сброса и не потерять ничего',
    color: '#dc2626',
    steps: [
      {
        title: 'Скачай базу данных (ОБЯЗАТЕЛЬНО)',
        description: 'Твоя база данных НЕ сохраняется в GitHub. Без этого файла -- все документы, заметки и инструкции пропадут при сбросе среды. Скачай ПРЯМО СЕЙЧАС:',
        codeBlocks: [
          { label: 'Скачать БД', code: 'Открой в браузере: /api/download-db\nФайл wiki-codex-backup.db скачается автоматически' },
        ],
      },
      {
        title: 'Сохрани файл в надёжное место',
        description: 'Загрузи скачанный файл на Google Drive, Dropbox или просто на компьютер. Это твоя страховка.',
        codeBlocks: [
          { label: 'Размер файла', code: '~100 KB -- весит почти ничего\nСодержит: все документы, заметки, инструкции, термины, категории, теги' },
        ],
      },
      {
        title: 'Если среда Z.ai сломалась -- восстановление',
        description: 'Клон из GitHub + положить скачанный .db файл + запустить:',
        codeBlocks: [
          { label: 'Шаг 1. Клон', code: 'git clone https://github.com/Sts8987/Wiki-Codex.git\ncd Wiki-Codex' },
          { label: 'Шаг 2. Зависимости', code: 'bun install' },
          { label: 'Шаг 3. Создать папку', code: 'mkdir -p db' },
          { label: 'Шаг 4. Положить БД', code: 'Перемести wiki-codex-backup.db в папку db/\nПереименуй в custom.db' },
          { label: 'Шаг 5. Запуск', code: 'bun run dev' },
        ],
      },
      {
        title: 'Проверь что всё работает',
        description: 'После запуска открой приложение и проверь:',
        codeBlocks: [
      ...
      {
        title: 'Регулярный бэкап',
        description: 'Скачивай базу данных после каждой важной работы. Это занимает 5 секунд:',
        codeBlocks: [
          { label: 'Создать бэкап', code: 'Открой в браузере: /api/backup\n(автоматически сохранит в db/backups/)' },
          { label: 'Скачать', code: 'Открой в браузере: /api/download-db\n(скачает файл на компьютер)' },
        ],
      },
    ],
  },
